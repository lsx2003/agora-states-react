...
export default function FormContainer() {
  ...
  const changeHandler = (event) => {
    if (event.target.id === "name") {
      ...
      console.log(name);
    } else if (event.target.id === "name2") {
      setTitle(event.target.value);
      console.log(title);
    }
  };

  useCallback(changeHandler, []);
    ...
    <section className="form__container">
      <div className="form__input--wrapper">
        <div className="form__input--name">
          ...
          <input onChange={changeHandler} type="text" id="name" required />
        </div>
        <div className="form__input--title">
          <label htmlFor="name2">제목을 입력하세요. </label>
          <input onChange={changeHandler} type="text" id="name2" required />
        </div>
